(ns soma.methods.social
  "social.cljc — soma DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from soma's own
...
(defn draft-observation-post
  "A dry-run observation post over soma's own domain (Forestry / logging robotics — the logging body the robotics remote-work survey (ADR-2606073001 §4) reserved (伐採, one of the deadliest…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "soma R0: live social posting is Council Lv6+ + operator + "
